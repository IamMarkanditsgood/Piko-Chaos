...
{
    [SerializeField] private RawImage[] _avatarImages;
    ...
    public void SetSavedPicture()
    {
        if (PlayerPrefs.HasKey("AvatarPath"))
        {
            ...
            foreach (RawImage avatar in _avatarImages)
            {
                avatar.texture = NativeGallery.LoadImageAtPath(path, maxSize);
            }
        }
        else
        {
